...
# Project
from twoface.data import star_to_apogeervdata
from twoface.db import numpy_adapt # just need to execute code

from apogeebh.db.connect import Base


__all__ = ['AllStar', 'AllVisit', 'StarResult', 'Status']
...
class StarResult(Base):
    ...
    star = relationship("AllStar", uselist=False)
    status = relationship("Status", uselist=False)

    def __repr__(self):
        return ("<JokerResult(apogee_id='{}', status='{}')>"
...
class AllStar(Base):
    ...
    id = Column(types.Integer, primary_key=True)
    result = relationship("StarResult", uselist=False)
    ...
    def __repr__(self):
        if self.result is not None:
            status = "status='{0}'".format(self.result.status.message)
        else:
            status = ""
        return ("<ApogeeStar(id='{0}', apogee_id='{1}'{2})>"
                .format(self.id, self.apogee_id, status))
...
class AllVisit(Base):
    ...
    allstar_id = Column('allstar_id', types.Integer,
                        ForeignKey('allstar.id'),
                        index=True)
    star = relationship("AllStar", backref=backref("visits"))
